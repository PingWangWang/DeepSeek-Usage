...
[![Version](https://img.shields.io/badge/version-1.38.3-blue)]()
...
- **费用、Token、缓存全掌握**：当日费用、当月费用、当月平均费用、当月用量、钱包余额，一目了然
...
3. 面板包含：
   - **汇总卡片**：当日费用、当月费用、当月平均费用、当月用量、钱包余额
...
### 月度统计（跨月趋势）
- 面板底部独立的「月度统计」区块，汇总跨多月的费用与 Token 趋势
- 默认仅统计**当年**（1 月 → 当前月），可通过区块内「起 / 止」两个下拉框任意设置起止月份范围（往前推 3 年，无未来月份）
- 汇总卡片：区间累计费用（含月均）、区间累计 Token、最高消费月、费用环比（较前一月）
- 趋势图：月度费用柱状图（柱顶标注当月费用）+ 月度 Token 折线双轴图
- 防频繁请求：仅首次展开 / 手动刷新 / 缓存过期 / 范围变更时拉取；30 分钟 TTL 缓存 + 3 并发限流，单月失败降级为 0
...
| 模型用量 / Key明细 / 每日明细 / 月度统计 | 开关按钮，独立控制各区块显示/隐藏，状态持久化 |
