...
- **费用、Token、缓存全掌握**：今日消费、月度费用、输入/输出均价、预估可用余额，一目了然
- **交互图表驱动分析**：基于 ECharts 实现请求趋势、Token 构成、缓存命中率、模型分布等 6 张交互图表
- **Key 级明细账单**：支持从 DeepSeek 导出 ZIP 导入 Key 级别的用量和费用数据，含堆叠条形图
...
3. 面板包含：
   - **汇总卡片**：今日消费、本月费用、均价、本月用量、预估可用 Tokens
   - **趋势图表**：API 请求数、Tokens、缓存命中率、Token 构成
   - **模型明细表**：每个模型的请求数、Tokens、缓存命中、费用
   - **模型分布图**：饼图展示各模型 Token 占比
4. 点击 **导入 Key 明细** 按钮，从 DeepSeek 导出 ZIP 导入 Key 级别费用数据
...
| 指标 | 说明 |
|------|------|
| 今日消费 | 当日实际费用（优先从 cost API 获取，fallback 按均价估算） |
| 本月费用 | 当前月份总费用 |
| 平均消费 | 每百万 Tokens 的均价，细分输入/输出 |
| 预估可用 | 根据钱包余额和均价估算的可用 Tokens |
...
| Key 费用分布 | 堆叠水平条形图 | 各 API Key 的未缓存 / 缓存 / 输出费用 |

### Key 明细（高级功能）
- 点击 **导入 Key 明细** 按钮，脚本会自动调用 DeepSeek 导出接口下载月度 ZIP
- 解压并解析其中的 `amount-*.csv` 文件，按 Key 聚合
- 支持缓存命中/未命中的费用拆分
- 表格可折叠，图表高度自动适配 Key 数量
...
- `DeepSeek Usage.txt` — 用户脚本主文件（~2885 行），包含全部逻辑、样式、模板和图表配置
